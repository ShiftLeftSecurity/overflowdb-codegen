...
//          }
      ...
      s"""package $nodesPackage
         ...
         |$mixinTraitsForBase
         |
         |trait ${className}New extends NewNode with TypedCloneable[${className}New]
         |$mixinsNew
         |$mixinTraitsNew
         |
  ...
  protected def writeNewNodeFile(outputDir: File): File = {
    val staticHeader =
      s"""package $nodesPackage
         ...
         |trait NewNode extends AbstractNode with TypedCloneable[NewNode]{
         |  // NewNode are done via case classes, in order to profit from a variety of autogenerated goodies.
         |  // however, NewNode are mutable and compare by identity. The latter fact is because they must
         |  // behave by-identity during diffgraph application
         |  override def equals(other:Any):Boolean = this eq other.asInstanceOf[java.lang.Object]
         |  override def hashCode():Int = java.lang.System.identityHashCode(this)
         |
         |  def properties: Map[String, Any]
         |}
         |trait TypedCloneable[+NodeType <: Object] extends scala.Cloneable {
         |  override def clone():NodeType = super.clone().asInstanceOf[NodeType]
         |}
         |
    ...
    def generateNewNodeSource(nodeType: NodeType, properties: Seq[Property[_]]) = {
      ...
      s"""
         ...
         |}
         |
         |case class New${nodeClassName} private[nodes] ($memberVariables)
         |  extends NewNode with ${nodeClassName}Base ${mixins} with TypedCloneable[New${nodeClassName}] {
         |
