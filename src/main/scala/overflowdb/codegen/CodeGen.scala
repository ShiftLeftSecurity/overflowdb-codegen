    ...
    def generateNodeSource(nodeType: NodeType) = {
      ...
      val companionObject =
        s"""object $className {
           ...
           |
           |  object Edges {
           |    val In: Array[String] = Array(${quoted(inEdges.map(_.edge.name).distinct).mkString(",")})
           |    val Out: Array[String] = Array(${quoted(outEdges.map(_.edge.name).distinct).mkString(",")})
           |  }
           |
