...
import overflowdb.schema.{Cardinality, Constant, PartialSchema, SchemaBuilder}
...
object TestSchema2 extends App {
  ...
  val baseApplied = BaseSchema.apply(builder)
  new CodeGen(baseApplied.build).run(new File("target"))
}

object BaseSchema extends PartialSchema {

  override def apply(builder: SchemaBuilder): SchemaBuilder = {
