...
/** TODOs
 * * ensure they only get added once -> withBuilder should just lazily enqueue additions...?
 * * make all SchemaEntries extend a common trait `SchemaEntry`?
 * * idea: use implicit schemaBuilder instead? redefine all methods from SchemaBuilder in PartialSchema, but add an implicit? implicit for lazy val?
 *  */
object BaseSchema {
//  def apply(builder: SchemaBuilder): SchemaBuilder = ???

  private val entries: mutable.Buffer[_] = mutable.ListBuffer.empty

  private def withBuilder[A](addEntry: SchemaBuilder => A): A = {
//    val entry = addEntry()
//    entry
    ???
  }

  lazy val name: Property = withBuilder(_.addNodeProperty("NAME", "string", Cardinality.One, "Name of represented object, e.g., method name (e.g. \"run\")"))
}
